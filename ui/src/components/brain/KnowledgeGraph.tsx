  ...
  return (
    <div className="relative h-full min-h-[430px] overflow-hidden rounded-[24px] bg-[#0b1526]">
      ...
      {nodes.map(node => (
        ...
        >
          <div
            className={`rounded-full ${node.tone} shadow-[0_0_0_12px_rgba(167,139,250,0.06)]`}
            style={{ width: node.size * 2, height: node.size * 2 }}
          />
          <div className="mt-2 whitespace-nowrap text-center text-[11px] font-medium text-slate-300">
            {node.label}
          </div>
        </motion.div>
      ))}
