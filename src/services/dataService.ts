...
export const LM_STUDIO_PRESETS: Record<string, LMStudioConfig> = {
  default: {
    baseUrl: "http://192.168.4.116:1234",
    apiKey: "sk-lm-7ZiXI8bi:8aI8KBHTidnfc7tjeZiB",
    model: "gpt-oss-20b-hermes_agent-tool-finetune_gguf",
  },
};

let activeLMStudio: LMStudioConfig = LM_STUDIO_PRESETS.default;

export function setLMStudioConfig(config: Partial<LMStudioConfig>): void {
  activeLMStudio = { ...activeLMStudio, ...config };
  try { localStorage.setItem("sw_lmstudio", JSON.stringify(activeLMStudio)); } catch { /* silent */ }
}

export function loadLMStudioConfig(): LMStudioConfig {
  try {
    const saved = localStorage.getItem("sw_lmstudio");
    if (saved) return JSON.parse(saved);
  } catch { /* silent */ }
  return LM_STUDIO_PRESETS.default;
}

export function getLMStudioConfig(): LMStudioConfig {
  return activeLMStudio;
}

/** Test connection to LM Studio (or any OpenAI-compatible endpoint) */
export async function testLMStudioConnection(config?: Partial<LMStudioConfig>): Promise<{ ok: boolean; models?: string[]; error?: string }> {
  const cfg = { ...activeLMStudio, ...config };
  const url = cfg.baseUrl.endsWith("/v1") ? `${cfg.baseUrl}/models` : `${cfg.baseUrl}/v1/models`;
  try {
    const ctrl = new AbortController();
    ...
    const models = (data.data || []).map((m: any) => m.id || m.model || m).filter(Boolean);
...
export async function chatWithLMStudio(messages: { role: string; content: string }[], config?: Partial<LMStudioConfig>): Promise<{ ok: boolean; content?: string; error?: string }> {
  const cfg = { ...activeLMStudio, ...config };
...
// Initialize LM Studio config from localStorage on module load
activeLMStudio = loadLMStudioConfig();
