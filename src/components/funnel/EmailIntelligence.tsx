...
const STORAGE_KEYS = {
  ...
  enrichment: "sw_email_enrichment",
} as const;
...
/** Verify an email via mailcheck.ai (free, no auth required) */
async function verifyEmailViaAPI(email: string): Promise<EmailResult["status"] | null> {
  try {
    ...
    // mailcheck.ai returns: status can be "valid", "invalid", "risky", "unknown"
    const status = data.status || data.result;
    if (status === "valid" || status === true) return "verified";
    if (status === "invalid" || status === false) return "invalid";
    if (status === "risky" || status === "catch_all" || status === "disposable") return "risky";
    return "unverified";
  } catch {
    return null;
  }
...
/** Try to find a GitHub profile by email */
async function searchGithubByEmail(email: string): Promise<GithubUser | null> {
  try {
    ...
    if (data.total_count > 0 && data.items?.length > 0) {
      return data.items[0] as GithubUser;
    }
    // fallback: search by username part
    const localPart = email.split("@")[0];
    if (!localPart) return null;
    const res2 = await fetch(`https://api.github.com/search/users?q=${encodeURIComponent(localPart)}`, {
      headers: { Accept: "application/vnd.github+json" },
    });
    if (!res2.ok) return null;
    const data2 = await res2.json();
    if (data2.total_count > 0 && data2.items?.length > 0) {
      return data2.items[0] as GithubUser;
    }
    return null;
  } catch {
    return null;
...
export default function EmailIntelligence() {
  ...
  const [enrichmentMap, setEnrichmentMap] = useState<Record<string, EnrichmentData>>(() =>
    loadStored<Record<string, EnrichmentData>>(STORAGE_KEYS.enrichment, {})
  );
  const [patterns] = useState<EmailPattern[]>(() =>
    loadStored<EmailPattern[]>(STORAGE_KEYS.patterns, [
      { pattern: "{first}.{last}", example: "john.smith@domain.com", frequency: 42, confidence: 94 },
      { pattern: "{first}{last}", example: "johnsmith@domain.com", frequency: 28, confidence: 88 },
      { pattern: "{f}{last}", example: "jsmith@domain.com", frequency: 15, confidence: 82 },
      { pattern: "{first}_{last}", example: "john_smith@domain.com", frequency: 8, confidence: 72 },
      { pattern: "{first}", example: "john@domain.com", frequency: 4, confidence: 65 },
      { pattern: "{last}", example: "smith@domain.com", frequency: 3, confidence: 58 },
    ])
  );

  // ─── Enrichment editing state ─────────────────────────────────────────────
  const [editingEnrichmentEmail, setEditingEnrichmentEmail] = useState<string | null>(null);
  const [manualEnrichment, setManualEnrichment] = useState({ title: "", company: "", linkedin: "" });
  const [verifyingEmail, setVerifyingEmail] = useState<string | null>(null);
  const [enrichingEmail, setEnrichingEmail] = useState<string | null>(null);
  ...
  // Persist enrichment
  useEffect(() => {
    saveStored(STORAGE_KEYS.enrichment, enrichmentMap);
  }, [enrichmentMap]);
  ...
  useEffect(() => {
    ...
    const preview = composeEmail(
      selectedTemplate.id,
      {
        firstName: recipient.firstName || "Alex",
        lastName: recipient.lastName,
        email: recipient.email || "alex@firm.com",
  ...
  const searchDomain = useCallback(async () => {
    ...
    try {
      ...
      if (targets.length > 0) {
        ...
        // No targets — show empty state, don't fabricate emails
        setResults([]);
      }
  ...
  /** Verify a single email using the real mailcheck.ai API */
  const verifyEmail = useCallback(async (email: string) => {
    setVerifyingEmail(email);
    const apiStatus = await verifyEmailViaAPI(email);
    setVerifyingEmail(null);

    if (apiStatus === null) {
      // API failed — mark as unverified with note
      setResults((prev) =>
        prev.map((r) => (r.email === email ? { ...r, status: "unverified" as const } : r))
      );
      alert("Verification service unavailable — check manually");
      return;
    }

    setResults((prev) =>
      prev.map((r) => (r.email === email ? { ...r, status: apiStatus } : r))
    );
  }, []);

  /** Enrich an email: try GitHub API, then allow manual entry */
  const enrichEmail = useCallback(async (email: string) => {
    setEnrichingEmail(email);
    const githubUser = await searchGithubByEmail(email);
    setEnrichingEmail(null);

    if (githubUser) {
      const localPart = email.split("@")[0] || "";
      const parts = localPart.split(/[._-]/).filter(Boolean);
      const inferredTitle = parts.length >= 2
        ? `${parts[0][0].toUpperCase() + parts[0].slice(1)} ${parts[1][0].toUpperCase() + parts[1].slice(1)}`
        : localPart;

      const data: EnrichmentData = {
        title: inferredTitle,
        company: email.split("@")[1]?.replace(/\.(com|io|ai|net|org)$/, "") || "",
        linkedin: "",
      ...
      // No GitHub profile found — open manual enrichment form
      setEditingEnrichmentEmail(email);
      setManualEnrichment({ title: "", company: "", linkedin: "" });
    }
  }, []);

  /** Save manually-entered enrichment data */
  const saveManualEnrichment = useCallback((email: string) => {
    if (!manualEnrichment.title && !manualEnrichment.company && !manualEnrichment.linkedin) {
      setEditingEnrichmentEmail(null);
      return;
    }
    const data: EnrichmentData = {
      title: manualEnrichment.title || "—",
      company: manualEnrichment.company || email.split("@")[1] || "—",
      linkedin: manualEnrichment.linkedin || "",
      source: "manual",
    };
    setEnrichmentMap((prev) => ({ ...prev, [email]: data }));
    setEditingEnrichmentEmail(null);
  ...
  /** Verify multiple emails in bulk using the real API */
  const verifyBulk = useCallback(async () => {
    ...
    for (const email of emails) {
      const apiStatus = await verifyEmailViaAPI(email);
      const status = apiStatus ?? "unverified";
      const reasons: Record<string, string> = {
        verified: "MX record found, SMTP accepted",
        unverified: "Domain valid, mailbox unchecked (or service unavailable)",
        invalid: "MX not found or SMTP rejected",
        risky: "Catch-all, disposable, or role-based address",
      };
      processed.push({ email, status, reason: reasons[status] || "Unknown" });
    }
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <div className="text-[11px] font-medium text-slate-200">
                        {recipient.firstName || "Alex"} {recipient.lastName} &lt;{recipient.email || "alex@firm.com"}&gt;
                      </div>
                      <div className="text-[10px] text-slate-500">
                        {recipient.firmName || "Example Ventures"} · {recipient.investorFocus || "enterprise AI"}
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
            ...
            {results.length === 0 && domain && !searching && (
              <div className="mt-4 p-6 rounded-xl border border-dashed border-white/[0.08] bg-white/[0.02] text-center">
                <Mail className="w-8 h-8 mx-auto mb-2 text-slate-600" />
                <div className="text-sm text-slate-400 font-medium">No emails found for this domain</div>
                <div className="text-[11px] text-slate-500 mt-1 max-w-md mx-auto">
                  Add targets to your campaign with decision maker info, or use the email composer to write to known contacts.
                </div>
                <div className="mt-3 flex items-center justify-center gap-2">
                  <button
                    onClick={() => setActiveTab("composer")}
                    className="text-[11px] px-3 py-1.5 rounded-lg bg-indigo-500 text-white font-medium hover:bg-indigo-600 transition-all"
                  >
                    Go to Composer
                  </button>
                </div>
                      ...
                      <tr key={r.email} className="text-xs border-t border-white/[0.06]">
                          ...
                          <div className="flex flex-col">
                            ...
                            {enrichmentMap[r.email] && (
                              <span className="text-[10px] text-slate-500 mt-0.5">
                                ...
                                {enrichmentMap[r.email].github && (
                                  <a
                                    href={enrichmentMap[r.email].github}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center gap-0.5 ml-2 text-sky-400 hover:underline"
                                  >
                        ...
                        <td className="py-2">
                          <button
                            ...
                            disabled={verifyingEmail === r.email}
                            className="text-[10px] text-indigo-400 hover:text-indigo-300 transition-colors mr-2 disabled:opacity-50"
                          >
                            {verifyingEmail === r.email ? "Checking..." : "Verify"}
                          </button>
                          <button
                            onClick={() => enrichEmail(r.email)}
                            disabled={enrichingEmail === r.email}
                            className="text-[10px] text-sky-400 hover:text-sky-300 transition-colors disabled:opacity-50"
                          >
                            {enrichingEmail === r.email ? "Searching..." : "Enrich"}
                          </button>
                        </td>
                      </tr>
                    ))}
            ...
            {/* Manual enrichment form modal */}
            {editingEnrichmentEmail && (
              <div className="mt-3 p-4 rounded-xl border border-sky-500/20 bg-sky-500/[0.03]">
                <div className="flex items-center justify-between mb-3">
                  <div className="text-[11px] font-medium text-sky-300">
                    Add Enrichment for {editingEnrichmentEmail}
                  </div>
                  <button
                    onClick={() => setEditingEnrichmentEmail(null)}
                ...
                <div className="text-[10px] text-slate-500 mb-3">
                  No public profile found via GitHub API. Add enrichment data manually — it will be saved to localStorage.
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <input
                    value={manualEnrichment.title}
                    onChange={(e) => setManualEnrichment((p) => ({ ...p, title: e.target.value }))}
                    placeholder="Job title (e.g. CTO)"
                    className="text-xs px-3 py-2 rounded-lg border border-white/[0.1] bg-[#0f172a] text-slate-100 outline-none focus:border-sky-500 placeholder:text-slate-600"
                  />
                  <input
                    value={manualEnrichment.company}
                    onChange={(e) => setManualEnrichment((p) => ({ ...p, company: e.target.value }))}
                    placeholder="Company name"
                    className="text-xs px-3 py-2 rounded-lg border border-white/[0.1] bg-[#0f172a] text-slate-100 outline-none focus:border-sky-500 placeholder:text-slate-600"
                  />
                  <input
                    value={manualEnrichment.linkedin}
                    onChange={(e) => setManualEnrichment((p) => ({ ...p, linkedin: e.target.value }))}
                ...
                <div className="flex items-center gap-2 mt-2">
                  <button
                    onClick={() => saveManualEnrichment(editingEnrichmentEmail)}
                    className="text-[11px] px-3 py-1.5 rounded-lg bg-sky-500 text-white font-medium hover:bg-sky-600 transition-all"
                  >
                    Save Enrichment
                  </button>
                  <button
                  ...
                  {(() => {
                    ...
                    return Object.entries(discoveredPatterns)
                      ...
                      .map(([pattern, data]) => {
                        ...
                        const commonPattern = patterns.find((p) => p.pattern === pattern);
                        return (
                              ...
                              <div className="flex flex-col">
                                ...
                                {commonPattern && (
                                  <span className="text-[9px] text-slate-600">
                                    Matches known pattern: {commonPattern.example}
                                  </span>
                                )}
                              </div>
                            </div>
