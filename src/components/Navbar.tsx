...
export default function Navbar({ view, onSwitchView }: NavbarProps) {
  ...
  const { campaigns, campaignId, health, lastRefresh } = state;
  // Ref-based live indicator: shows if backend is connected (avoids re-render from isPolling toggle)
  const isLive = !!lastRefresh;
  ...
  return (
    <nav className="h-14 border-b border-white/[0.08] flex items-center px-4 gap-4 shrink-0" style={{ background: "#08111f" }}>
      ...
      {isLive && (
        <span className="flex items-center gap-1.5 text-xs text-emerald-400" title="Receiving live updates">
